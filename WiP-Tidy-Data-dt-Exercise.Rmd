...
```
```{r}
wip[, c("Indicatoe.Name", "Indicator.Code", "V65"):=NULL]
...
WP[Country %in% dclpct] %>%
  ...
  ggtitle("Women in Parliment: Decline >=5") +
  ylab("% Women in Parliment")
```
...
cWP[Country %in% c("Portugal", "Sweden", "Spain", "Hungary", "Romania", "Finland", "Germany")] %>%
  ...
  ggtitle("Women in Parliment: Ranked") +
  ylab("Rank in Europe")
```
...
cWP[is.na(Continent)] %>%
  ...
  scale_y_continuous(limits = c(0,40),
                     breaks=seq(0, 40, by=10)) +
  ggtitle("Women in Parliment: Global Trends") +
  ylab("Women in Parliment")
```
